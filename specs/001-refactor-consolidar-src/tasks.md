...
### Pre-implementación
- **T000** (US: —) Generar `specs/001-refactor-consolidar-src/baseline-coverage.md` (% total, 9 módulos, comando reproducible). ✓ archivo existe antes de slice 1.
...
- **T009** (US-2, US-4) Imports full-path cross-domain; eliminar alias `PersonTracker`; config + graph. ✓ gates verdes; callers usan `PlayerTracker`.
...
- **T012.5** (US-2) Migrar imports bare restantes a full-path para módulos no movidos (`src.schema`, `src.analytics`, `src.pipeline`, `src.data`, `src.detectors`) en cualquier archivo del repo. ✓ `git grep -E "^from (schema|analytics|pipeline|data|detectors)"` → 0 hits fuera de imports ya full-path bajo `src/` mismo.
